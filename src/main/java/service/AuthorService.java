...
public class AuthorService {
    ...
    public void numberOfFemaleGender() {
        ...
        Optional<Integer> result = Optional.ofNullable(authors.stream()
                .filter(author -> author.getGender().equals("Female"))
                .map(author -> 1)
                .reduce(0, (number1, number2) -> number1 + number2));

        if (result.isPresent()) {
            System.out.println("Number of females : " + result.get());
        }
    ...
    public void transformAuthorsInArticles() {
        ...
        System.out.println(articleServiceUseful.displayInformationAboutArticles((ArrayList<Article>) articles));
    }
    ...
    public void findArticlesByAuthorName(String authorName) {
        ...
        Optional<Article> article = authors.stream()
                .filter(author -> author.getNameAuthor().equals(authorName))
                .map(Author::getArticles)
                .flatMap(List::stream)
                .findFirst();
